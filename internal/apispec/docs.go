...
func LLMsTXT(baseURL string) string {
	...
	fmt.Fprintf(&b, "## Showing your work: a preview URL\n\n")
	fmt.Fprintf(&b, "An agent that changed an application can prove it runs, and hand a person a link instead of a diff.\n\n")
	fmt.Fprintf(&b, "1. Build the application image and push it to your registry. Kiln does not build application images.\n")
	fmt.Fprintf(&b, "2. Store the registry credential once with `POST /v1/registries`. The host pulls with it; the sandbox never holds it, because the sandbox runs the code under review.\n")
	fmt.Fprintf(&b, "3. `POST /v1/templates` with that image, an `egress_allow` list naming every host the application needs, and `ttl_seconds` so the preview ends even if nobody comes back for it.\n")
	fmt.Fprintf(&b, "4. `POST /v1/sandboxes` with `lifecycle: persistent`, an `idle_seconds` deadline, and `metadata` naming the change: the branch, the pull request, the commit. The metadata is how you find this preview again.\n")
	fmt.Fprintf(&b, "5. `POST /v1/sandboxes/{id}/publish` returns the URL. Give that to the person.\n\n")
	fmt.Fprintf(&b, "The preview sleeps after its idle deadline and wakes on the next request, so an open review costs nothing while nobody is looking. The host deletes the template at its TTL with the sandbox and the snapshot, so a forgotten preview cleans itself up.\n\n")
	fmt.Fprintf(&b, "Egress is deny by default. A preview that reaches a database or an API needs those hostnames in `egress_allow`, and nothing else is reachable from inside it.\n\n")
